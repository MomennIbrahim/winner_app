...
class FourTeamsHome extends StatelessWidget {
  const FourTeamsHome({super.key, required this.isCheck});
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AppCubit, AppState>(
      builder: (context, state) {
        if (!isCheck) {
          if (context.read<AppCubit>().totalAScore >=
              ...
              context.read<AppCubit>().totalCScore >=
                  int.parse(context.read<AppCubit>().topScoreController.text) ||
              context.read<AppCubit>().totalDScore >=
                  int.parse(context.read<AppCubit>().topScoreController.text)) {
        ...
        var tDListScore = context.read<AppCubit>().tDListScore;
                ...
                child: Column(
                  ...
                  children: [
                    FittedBox(
                      fit: BoxFit.scaleDown,
                      child: Text(
                        context.read<AppCubit>().tDController.text.isNotEmpty
                            ? context.read<AppCubit>().tDController.text
                            : "Team D",
                    ...
                    Expanded(
                      child: Column(
                        children: List.generate(
                          context.read<AppCubit>().tDListScore.length,
                          (index) => Padding(
                            padding: EdgeInsets.only(bottom: 4.0.h),
                            child: Row(
                              children: [
                                if (tDListScore.last.toString() ==
                                    tDListScore[index].toString())
                                  GestureDetector(
                                    onTap: () {
                                      context
                                          .read<AppCubit>()
                                ...
                                Expanded(
                                  child: Padding(
                                    padding: tDListScore.last.toString() ==
                                            tDListScore[index].toString()
                                        ? EdgeInsets.only(right: 24.w)
                                        : const EdgeInsets.only(),
                                    child: FittedBox(
